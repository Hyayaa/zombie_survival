...
describe("knife melee range", () => {
  it("uses the reduced final range and bounded width profiles", () => {
    expect(MELEE_ATTACK_DEFINITIONS.knife.stab.range).toBe(13);
    expect(MELEE_ATTACK_DEFINITIONS.knife.swing.range).toBe(14);
    expect(MELEE_ATTACK_DEFINITIONS.knife.heavy.range).toBe(15);
  ...
  it("matches the swing pixel radius to the 14px hit profile", () => {
    ...
    if (trail.geometry.kind !== "crescent") throw new Error("expected crescent");
    expect(trail.geometry.outerRadius).toBe(14);
    expect(trail.geometry.outerRadius - MELEE_ATTACK_DEFINITIONS.knife.swing.range).toBeLessThanOrEqual(1);
  });

  it("does not reach a target beyond profile plus target radius and preserves line-of-sight blocking", () => {
    const target = { id: "zombie", position: { x: 23, y: 0 }, alive: true };
    expect(collectMeleeTargets({ x: 0, y: 0 }, 0, MELEE_ATTACK_DEFINITIONS.knife.stab, [target], () => true, [])).toHaveLength(0);
    const closeTarget = { ...target, position: { x: 18, y: 0 } };
