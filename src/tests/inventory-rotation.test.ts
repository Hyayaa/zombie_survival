...
describe("inventory item rotation", () => {
  it("swaps effective dimensions and rotates in place atomically", () => {
    expect(getEffectiveFootprint(getItemDefinition("water"), 1)).toEqual({ width: 2, height: 1 });
    ...
    expect(inventory.getItem(water.instanceId)).toMatchObject({ rotation: 1, width: 2, height: 1, x: water.x, y: water.y });
  });
  it("tries base rotation before rotated auto-placement", () => {
    ...
    expect(inventory.getStoredItems().find((item) => item.itemId === "water")).toMatchObject({ rotation: 1, width: 2, height: 1 });
  });
});
