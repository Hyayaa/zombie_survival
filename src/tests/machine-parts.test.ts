...
describe("machine parts",()=>{
  ...
  it("defines and distributes every part with enough material for all four kits",()=>{for(const id of ids)expect(ITEM_DEFINITIONS[id]?.category).toBe("material");const totals:Record<string,number>={};for(const c of createCityBlockMap(77).containers)for(const l of c.loot)totals[l.itemId]=(totals[l.itemId]??0)+l.quantity;expect(totals).toMatchObject({screws:20,steel_plate:9,solar_panel:2,duct_tape:4,circuit_board:2,electric_motor:2});});
