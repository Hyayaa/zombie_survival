...
import { getCameraChunkKey, type WorldViewRect } from "./world-chunk-visibility";
...
export class StructureChunkRenderer{
  private readonly states=new Map<string,PlacedStructureState>();private readonly chunks=new Map<string,{graphics:Phaser.GameObjects.Graphics;ids:Set<string>;x:number;y:number}>();private lastViewportKey=Number.NaN;private minVisibleX=0;private minVisibleY=0;private maxVisibleX=-1;private maxVisibleY=-1;
  constructor(private readonly scene:Phaser.Scene){}
  add(state:PlacedStructureState):StructureRuntimeView{this.states.set(state.id,state);for(const key of keysFor(state)){let chunk=this.chunks.get(key);if(!chunk){const [x,y]=key.split(",").map(Number);chunk={graphics:this.scene.add.graphics().setDepth(DEPTH.propFront).setVisible(this.isVisibleChunk(x!,y!)),ids:new Set(),x:x!,y:y!};this.chunks.set(key,chunk);}chunk.ids.add(state.id);this.redraw(key);}return new ChunkHandle(state.id,this);}
  updateViewport(view:WorldViewRect):boolean{const key=getCameraChunkKey(view,CHUNK_TILES,TILE_SIZE);if(key===this.lastViewportKey)return false;this.lastViewportKey=key;this.minVisibleX=Math.floor(view.x/CHUNK_SIZE)-1;this.minVisibleY=Math.floor(view.y/CHUNK_SIZE)-1;this.maxVisibleX=Math.floor((view.x+Math.max(0,view.width-1))/CHUNK_SIZE)+1;this.maxVisibleY=Math.floor((view.y+Math.max(0,view.height-1))/CHUNK_SIZE)+1;for(const chunk of this.chunks.values())chunk.graphics.setVisible(this.isVisibleChunk(chunk.x,chunk.y));return true;}
  ...
  private isVisibleChunk(x:number,y:number):boolean{return x>=this.minVisibleX&&x<=this.maxVisibleX&&y>=this.minVisibleY&&y<=this.maxVisibleY;}
}
