...
export class StructureChunkRenderer{
  ...
  private redraw(key:string):void{const chunk=this.chunks.get(key);if(!chunk)return;chunk.graphics.clear();for(const id of chunk.ids){const state=this.states.get(id);if(!state||state.placement.kind!=="segment")continue;const p=state.placement,definition=BUILDABLE_DEFINITIONS[state.kind],ratio=state.health/state.maximumHealth,color=state.kind==="metal-wall"?0x667174:0x755235;chunk.graphics.lineStyle(definition.segment!.thickness+2,0x111612,1).lineBetween(p.startX,p.startY,p.endX,p.endY).lineStyle(definition.segment!.thickness,color,1).lineBetween(p.startX,p.startY,p.endX,p.endY).lineStyle(1,state.kind==="metal-wall"?0xa4b0b0:0xa77b4d,1).lineBetween(p.startX,p.startY-1,p.endX,p.endY-1);if(ratio<=.7){const x=(p.startX+p.endX)/2,y=(p.startY+p.endY)/2;chunk.graphics.fillStyle(ratio<=.35?0x171c19:0x40372c).fillRect(Math.round(x)-1,Math.round(y)-1,ratio<=.35?3:2,2);}}}
}
