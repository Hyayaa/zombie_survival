...
export function getBgmTargetVolume(pressure:number,ducked=false):number{const threat=Math.max(0,Math.min(1,pressure/6));const volume=BGM_DEFINITION.volume+(BGM_DEFINITION.maxThreatVolume-BGM_DEFINITION.volume)*threat;return ducked?volume*.62:volume;}
export class AudioSystem {
  private readonly pools=new Map<AudioCue,Phaser.Sound.BaseSound[]>(); private readonly lastPlayedAt=new Map<AudioCue,number>(); private readonly eventGate=new AudioEventGate(); private bgm?:Phaser.Sound.BaseSound; private variantSequence=0; private destroyed=false;private pendingDayStart=false;private userActivated=false;private duckUntil=0;private bgmVolume=BGM_DEFINITION.volume;
  ...
  unlockAndStartBgm():void{if(this.destroyed)return;this.userActivated=true;try{if(this.scene.sound.locked)this.scene.sound.unlock();if(this.scene.sound.locked)return;if(!this.bgm){this.bgm=this.scene.sound.add(BGM_DEFINITION.asset,{loop:true,volume:this.bgmVolume});this.bgm.play();}else if(!this.bgm.isPlaying)this.bgm.play();if(this.pendingDayStart){this.pendingDayStart=false;this.playDayStartNow();}}catch{/* Optional audio can be rejected by autoplay policy or a missing decoder. */}}
  requestDayStart():void{if(this.destroyed)return;if(!this.userActivated||this.scene.sound.locked){this.pendingDayStart=true;return;}this.playDayStartNow();}
  updateBgm(pressure:number,deltaSeconds:number):void{if(!this.bgm)return;const target=getBgmTargetVolume(pressure,this.scene.time.now<this.duckUntil);const amount=Math.min(1,Math.max(0,deltaSeconds)*3.5);this.bgmVolume+= (target-this.bgmVolume)*amount;(this.bgm as unknown as {setVolume:(value:number)=>unknown}).setVolume(this.bgmVolume);}
  ...
  destroy():void{if(this.destroyed)return;this.destroyed=true;this.pendingDayStart=false;this.bgm?.stop();this.bgm?.destroy();for(const pool of this.pools.values())for(const sound of pool){sound.stop();sound.destroy();}this.pools.clear();this.eventGate.clear();}
  private playDayStartNow():void{if(this.play("day-start"))this.duckUntil=this.scene.time.now+1_000;}
}
