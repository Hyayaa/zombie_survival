...
export interface FootstepDustParticlePlan { velocityX: number; velocityY: number; lifetimeMs: number; size: 1 | 2 | 3; color: number }
...
export function createFootstepDustPlan(sequence: number, movementAngle: number, running: boolean, terrain: FootstepDustTerrain): FootstepDustParticlePlan[] {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const lifetimeMs = running ? 240 + random(seed, 2 + index * 4) * 180 : 180 + random(seed, 2 + index * 4) * 140;
    const travel = running ? 5 + random(seed, 3 + index * 4) * 9 : 2 + random(seed, 3 + index * 4) * 6;
    const speed = travel / (lifetimeMs / 1_000 * .7);
    const size = (running ? 1 + Math.floor(random(seed, 4 + index * 4) * 3) : 1 + Math.floor(random(seed, 4 + index * 4) * 2)) as 1 | 2 | 3;
